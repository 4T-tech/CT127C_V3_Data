...
uint8_t  ucHour=23, ucMin=59, ucSec=55;
/* USER CODE END PV */
...
void OLED_Proc(void)                	/* OLED处理 */
{
  ...
  usAdc[2] = usAdc[2]*330/4095;
  if(ucState == 0)
  {
    OLED_ShowString(0, 0, (uint8_t*)"       RTC   ", 16);
    sprintf((char*)ucBuf, "    %02u-%02u-%02u ", ucHour, ucMin, ucSec);
    OLED_ShowString(0, 2, ucBuf, 16);
  } else {
    sprintf((char*)ucBuf, "   VRP1:%04.2fV  ", usAdc[0]/100.0);
    OLED_ShowString(0, 0, ucBuf, 16);
    sprintf((char*)ucBuf, "   VRP2:%04.2fV  ", usAdc[2]/100.0);
    OLED_ShowString(0, 2, ucBuf, 16);
  }
}
  ...
  {                                   	/* 接收6个字符 */
    ...
    if(ucLrx[3]=='1')
      sprintf((char*)ucBuf, "VRP1:%4.2f:%02u-%02u-%02u*",
        usAdc[0]/100.0, ucHour, ucMin, ucSec);
    else
      sprintf((char*)ucBuf, "VRP2:%4.2f:%02u-%02u-%02u*",
        usAdc[2]/100.0, ucHour, ucMin, ucSec);
    LORA_Tx(ucBuf, 19);
  }
}
/* USER CODE END 4 */
