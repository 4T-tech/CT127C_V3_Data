...
void KEY_Proc(void)                 	/* 按键处理 */
{
  ...
  switch(ucKdn)                     	/* 按键按下 */
  {
    case 'B'+1:
      ...
      break;
    case 'B'+2:
      sprintf((char*)ucBuf, "*2%03u", ucAddr);
      LORA_Tx(ucBuf, 5);           	/* 发送查询指令 */
      break;
    case 'B'+4:
      if(ucState == 0)
      {
        sprintf((char*)ucBuf, "*4%03u", ucAddr);
        LORA_Tx(ucBuf, 5);          	/* 发送控制指令 */
      } else
        ucAddr++;
      break;
    case 'B'+5:
      if(ucState == 0)
      {
        sprintf((char*)ucBuf, "*5%03u", ucAddr);
        LORA_Tx(ucBuf, 5);          /* 发送控制指令 */
      } else
        ucAddr--;
  }
...
void LORA_Proc(void)                	/* LoRa处理 */
{
  if((LORA_Rx(ucLrx)==0) && (ucLrx[2]=='*'))
    ucPara = (ucLrx[0]-0x30)*10+ucLrx[1]-0x30;
}
/* USER CODE END 4 */
